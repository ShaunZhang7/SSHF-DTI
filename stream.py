...
def protein2emb_encoder(x):
    ...
    try:
        # i1 = np.asarray([words2idx_p[i] for i in t1]) # index——将t1中的每个单词（或标记）转换为相应的索引，通过words2idx_p字典实现，该字典将每个单词映射到一个唯一的整数索引
    ...
    if l < max_p:  # 如果序列长度小于max_p，则用0将i1填充到max_p；创建一个input_mask列表用于指示哪些部分是原始序列，哪些部分是填充的
        ...
        i = i1[:max_p]
        input_mask = [1] * max_p
        #print("protein over 1010: "+ str(l + 1))
...
class BIN_Data_Encoder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        p_v, input_mask_p = protein2emb_encoder(
            p)  # 使用protein2emb_encoder函数将'Target Sequence'字符串p编码为嵌入向量p_v和相应的输入掩码input_mask_p
